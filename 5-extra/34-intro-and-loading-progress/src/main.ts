...
import {
  ...
  PerspectiveCamera, PlaneBufferGeometry, ReinhardToneMapping,
...
const loadingManager    = new LoadingManager(
  () => {
    gsap.delayedCall(0.5, () => {
      gsap.to(overlayMaterial.uniforms.uAlpha, {duration: 3, value: 0, delay: 0.5});
      loadingBarElement.classList.add('ended');
      loadingBarElement.style.transform = '';
    });
  },
  (_, itemsLoad, itemsTotal) => {
    const progressRatio               = itemsLoad / itemsTotal;
 ...
 * Overlay
 */
const overlayGeometry = new PlaneBufferGeometry(2, 2);
